...
function normalizeOpenAiServiceTier(payload) {
...
function stripOpenAiServiceTier(payload) {
  if (!payload || typeof payload !== 'object' || !Object.prototype.hasOwnProperty.call(payload, 'service_tier')) return payload;
  const next = { ...payload };
...
function requestOptionsWithBodyLength(options = {}, body = '') {
  const next = {
    ...options,
    headers: { ...(options.headers || {}) },
  };
  const hasContentLength = Object.keys(next.headers).some((key) => String(key).toLowerCase() === 'content-length');
  if (body && !hasContentLength) {
    next.headers['content-length'] = Buffer.byteLength(body);
  }
  return next;
}
...
function requestJson(url, options = {}, body = '', timeoutMs = UPSTREAM_TIMEOUT_MS) {
  return new Promise((resolve, reject) => {
    ...
    const req = client.request(url, requestOptionsWithBodyLength(options, body), (res) => {
      let text = '';
...
function shouldRetryWithoutServiceTier(response, upstreamPayload) {
  if (!upstreamPayload || !Object.prototype.hasOwnProperty.call(upstreamPayload, 'service_tier')) return false;
  const status = Number(response?.statusCode || 0);
  if (![400, 422].includes(status)) return false;
  const err = extractUpstreamError(response);
  const text = `${err.code} ${err.type} ${err.message}`;
  return text.includes('service_tier')
    || text.includes('service tier')
    || text.includes('priority')
    || text.includes('unsupported');
}

async function requestOpenAiCompatibleResponse(payload, runtime) {
  const upstreamPayload = normalizeOpenAiServiceTier({
    ...(payload || {}),
    stream: false,
  });
  appendDebugLog({
  ...
  if (shouldRetryWithoutServiceTier(upstreamResponse, upstreamPayload)) {
    const standardPayload = stripOpenAiServiceTier(upstreamPayload);
    appendDebugLog({
      phase: 'upstream_request',
      endpoint: 'responses_service_tier_retry_standard',
      upstreamKind: upstreamKind(runtime),
      model: standardPayload.model || '',
      body: standardPayload,
    });
    const standardResponse = await requestJson(responsesUrl, {
      method: 'POST',
      headers: baseHeaders,
    ...
    appendDebugLog({
      ...
      body: standardResponse.json || standardResponse.text || null,
    });
    if (standardResponse.statusCode >= 200 && standardResponse.statusCode < 300 && standardResponse.json) {
      appendUsageRecord(runtime, extractResponseUsage(standardResponse.json), {
        endpoint: 'responses',
        model: standardResponse.json.model || standardPayload.model || '',
      });
      return { ok: true, responseJson: standardResponse.json };
    }
    if (!shouldFallbackToChatCompletions(standardResponse)) {
      return { ok: false, upstreamResponse: standardResponse };
    }
    return requestOpenAiCompatibleChatFallback(standardPayload, runtime);
  }
...
async function requestOpenAiCompatibleChatFallback(upstreamPayload, runtime) {
  const normalizedPayload = normalizeOpenAiServiceTier(upstreamPayload);
  const chatPayload = translateResponsesRequestToChatCompletions(normalizedPayload, runtime);
  const baseHeaders = {
    authorization: `Bearer ${runtime.upstream.apiKey}`,
    'content-type': 'application/json',
    accept: 'application/json',
  ...
  if (shouldRetryWithoutServiceTier(chatResponse, chatPayload)) {
    const standardChatPayload = stripOpenAiServiceTier(chatPayload);
    appendDebugLog({
      phase: 'upstream_request',
      endpoint: 'chat_completions_fallback_service_tier_retry_standard',
      upstreamKind: upstreamKind(runtime),
      model: standardChatPayload.model || '',
      toolCount: Array.isArray(standardChatPayload.tools) ? standardChatPayload.tools.length : 0,
      toolChoice: standardChatPayload.tool_choice ?? null,
      messageCount: Array.isArray(standardChatPayload.messages) ? standardChatPayload.messages.length : 0,
      body: standardChatPayload,
    });
    const standardChatResponse = await requestJson(chatUrl, {
      method: 'POST',
      headers: baseHeaders,
    }, JSON.stringify(standardChatPayload));
    appendDebugLog({
      phase: 'upstream_response',
      endpoint: 'chat_completions_fallback_service_tier_retry_standard',
      statusCode: standardChatResponse.statusCode,
      body: standardChatResponse.json || standardChatResponse.text || null,
    });
    if (standardChatResponse.statusCode >= 200 && standardChatResponse.statusCode < 300 && standardChatResponse.json) {
      const responseJson = chatCompletionsToOpenAiResponse(standardChatResponse.json, standardChatPayload.model);
      appendUsageRecord(runtime, responseJson.usage, {
        endpoint: 'chat_completions_fallback',
        model: responseJson.model || standardChatPayload.model || '',
      });
      return {
        ok: true,
        responseJson,
    ...
    return { ok: false, upstreamResponse: standardChatResponse };
  }
