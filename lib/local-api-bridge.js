...
async function requestOpenAiCompatibleResponsesOnly(upstreamPayload, runtime, options = {}) {
  const endpointLabel = String(options.endpointLabel || 'responses');
  appendDebugLog({
  ...
  if (endpointLabel === 'responses' && shouldRetryWithoutServiceTier(upstreamResponse, upstreamPayload)) {
    return requestOpenAiCompatibleResponsesOnly(stripOpenAiServiceTier(upstreamPayload), runtime, {
      endpointLabel: 'responses_service_tier_retry_standard',
    });
  }
...
async function requestOpenAiCompatibleChatFallback(upstreamPayload, runtime, options = {}) {
  const chatPayload = translateResponsesRequestToChatCompletions(normalizeOpenAiServiceTier(upstreamPayload), runtime);
  const baseHeaders = {
    authorization: `Bearer ${runtime.upstream.apiKey}`,
    'content-type': 'application/json',
    accept: 'application/json',
  ...
  if (shouldRetryWithoutServiceTier(chatResponse, chatPayload)) {
    ...
    const retryLabel = endpointLabel === 'chat_completions_fallback'
      ? 'chat_completions_fallback_service_tier_retry_standard'
      : `${endpointLabel}_service_tier_retry_standard`;
    appendDebugLog({
      phase: 'upstream_request',
      endpoint: retryLabel,
      upstreamKind: upstreamKind(runtime),
    ...
    if (standardChatResponse.statusCode >= 200 && standardChatResponse.statusCode < 300 && standardChatResponse.json) {
      ...
      return { ok: true, responseJson, chatPayload: standardChatPayload };
    }
    return { ok: false, upstreamResponse: standardChatResponse, chatPayload: standardChatPayload };
  }
