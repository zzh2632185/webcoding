...
function createAgentRuntime(deps) {
  ...
  function buildPiEnv() {
    const overrides = {};
    for (const key of PI_PASSTHROUGH_ENV_KEYS) {
      const value = processEnv[key];
      if (value !== undefined && value !== null && String(value) !== '') {
        overrides[key] = String(value);
      }
    }
    return sanitizeChildEnv(overrides);
  }
  ...
  /**
   * Pi coding agent (`pi -p --mode json`).
   * Prompt on stdin; multi-turn via --session-id + per-webcoding session-dir.
   * Attachments passed as @path CLI args (Pi convention).
   * Channel:
