...
function createAgentRuntime(deps) {
  ...
  // Quiet-by-default unknown event tracking (avoid log spam / UI noise).
  const reportedUnknownEventTypes = new Set();
  const reportedInteractiveEventTypes = new Set();

  // Strict interactive-event markers (word-boundary; avoid matching "task"/"mask").
  const INTERACTIVE_EVENT_RE = /(^|[._-])(permission|approval|elicitation|user_input|ask_user|request_user|auth_request|tool_permission|can_use_tool)([._-]|$)/i;

  function looksLikeInteractiveRuntimeEvent(event) {
    const type = String(event?.type || '');
    if (INTERACTIVE_EVENT_RE.test(type)) return true;
    const subtype = String(event?.subtype || event?.request_type || '');
    if (INTERACTIVE_EVENT_RE.test(subtype)) return true;
    return false;
  }

  function reportUnknownRuntimeEvent(entry, sessionId, agent, event) {
    const type = String(event?.type || 'unknown');
    // Log once per agent:type (global). UI tip once per session+type.
    ...
    if (!looksLikeInteractiveRuntimeEvent(event)) return;
    const uiKey = `${sessionId || 'none'}:${agent}:${type}`;
    ...
    sendSessionEvent(entry, sessionId, {
      type: 'system_message',
      message: [
        `${label} 发出交互事件「${type}」，当前 headless 适配层尚不能双向回应。`,
        '请停止本轮任务，并在终端原生 CLI 中完成该交互；或使用不触发该交互的权限模式。',
      ].join('\n'),
    });
  }
  ...
  function processClaudeEvent(entry, event, sessionId) {
    ...
    switch (event.type) {
      case 'system':
        ...
        if (event.subtype && looksLikeInteractiveRuntimeEvent(event)) {
          reportUnknownRuntimeEvent(entry, sessionId, 'claude', event);
        }
        break;
  ...
  function processCodexEvent(entry, event, sessionId) {
    ...
    switch (event.type) {
      ...
      default:
        reportUnknownRuntimeEvent(entry, sessionId, 'codex', event);
        break;
    }
  }
