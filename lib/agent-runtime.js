...
function resolveWindowsCommandPath(command) {
  ...
  const raw = stripCommandQuotes(command);
  if (!raw) return '';
  ...
  const pathExts = String(process.env.PATHEXT || '.COM;.EXE;.BAT;.CMD')
    ...
    .filter(Boolean);
  const preferredExts = ['.exe', '.cmd', '.bat', '.ps1', ...pathExts, ''];
  const uniqueExts = Array.from(new Set(preferredExts));
  const candidates = [];
  if (hasPathSeparator) {
    candidates.push(raw);
    if (!path.extname(raw)) {
      for (const ext of uniqueExts) if (ext) candidates.push(`${raw}${ext}`);
    }
  } else {
    const pathDirs = String(process.env.PATH || '').split(path.delimiter).filter(Boolean);
    for (const dir of pathDirs) {
      for (const ext of uniqueExts) if (ext) candidates.push(path.join(dir, `${raw}${ext}`));
...
function parseWindowsNpmCmdShim(commandPath) {
  if (process.platform !== 'win32' || !/\.cmd$/i.test(String(commandPath || ''))) return null;
  let source = '';
  try {
  ...
  const targets = [];
  const targetRe = /"%dp0%\\([^\"]+)"/gi;
  let match = null;
  while ((match = targetRe.exec(source))) targets.push(path.join(basedir, match[1]));
  const jsTarget = targets.find((target) => /\.(?:cjs|mjs|js)$/i.test(target));
  if (jsTarget) {
    const localNode = targets.find((target) => /node\.exe$/i.test(target) && fs.existsSync(target));
    return { command: localNode || process.execPath, argsPrefix: [jsTarget] };
  }
  const exeTarget = targets.find((target) => /\.exe$/i.test(target) && !/node\.exe$/i.test(target));
  if (exeTarget) return { command: exeTarget, argsPrefix: [] };
  return null;
}

function buildCliSpawnCommand(command, args) {
  const normalizedCommand = stripCommandQuotes(command);
  if (process.platform === 'win32' && /\.(?:cjs|mjs|js)$/i.test(normalizedCommand)) {
    return { command: process.execPath, args: [normalizedCommand, ...args], useShell: false };
  }
  if (process.platform === 'win32') {
    const resolvedCommand = resolveWindowsCommandPath(normalizedCommand);
    const shim = parseWindowsNpmCmdShim(resolvedCommand);
    if (shim) return { command: shim.command, args: [...shim.argsPrefix, ...args], useShell: false };
    if (/\.exe$/i.test(resolvedCommand)) return { command: resolvedCommand, args, useShell: false };
  }
  return { command, args, useShell: shouldUseShellForCommand(command) };
}

function withoutClaudeOneMillionContext(model) {
  const value = String(model || '').trim();
  return value.replace(/\[1m\]$/i, '');
}

function withClaudeOneMillionContext(model) {
  const value = withoutClaudeOneMillionContext(model);
...
function createAgentRuntime(deps) {
  ...
  const baseSendEntryMessage = typeof sendRuntimeMessage === 'function'
    ? sendRuntimeMessage
    : ((entry, data) => wsSend(entry?.ws, data));
  const sendEntryMessage = (entry, data = {}) => {
    const payload = (entry?.agent && !data.agent)
      ? { ...data, agent: entry.agent }
      : data;
    return baseSendEntryMessage(entry, payload);
  };
  ...
  function sanitizeChildEnv(extra = {}) {
    ...
    for (const key of Object.keys(env)) {
      ...
      if (key === 'OPENAI_API_KEY' || key === 'OPENAI_BASE_URL' || key === CODEX_OPENAI_COMPAT_ENV_KEY) delete env[key];
    }
  ...
  function buildClaudeSpawnSpec(session, options = {}) {
    ...
    const activeTemplate = modelCfg.mode === 'custom' && modelCfg.activeTemplate
      ...
      : null;
    const configuredModel = String(session.model || activeTemplate?.defaultModel || '').trim();
    const effectiveModel = configuredModel
      ? (activeTemplate ? withClaudeOneMillionContext(configuredModel) : configuredModel)
      : null;
    ...
    const shouldDowngradeRootYolo = requestedPermMode === 'yolo'
      && process.platform !== 'win32'
      && typeof process.getuid === 'function'
      && process.getuid() === 0;
    const permMode = shouldDowngradeRootYolo ? 'default' : requestedPermMode;
    if (shouldDowngradeRootYolo) {
      const rootWarning = '检测到 Webcoding 正以 root 用户运行，Claude 的 YOLO 模式会被 CLI 拒绝；已自动降级为默认模式。';
  ...
  function buildCodexSpawnSpec(session, options = {}) {
    ...
    const fastModeEnabled = String(processEnv.CC_WEB_CODEX_FAST_MODE_ENABLED || '').trim() === '1';
    const fastMode = fastModeEnabled && session.fastMode === true;
    if (!useAppServer) {
    ...
    if (runtimeConfig?.mode === 'custom') {
      args.push(
        ...
        '-c', `model_providers.openai_compat.env_key=${quoted(CODEX_OPENAI_COMPAT_ENV_KEY)}`,
    ...
    return {
      ...
      runtimeFingerprint,
      bridgeToken: runtimeConfig?.mode === 'custom' ? runtimeConfig.apiKey : null,
      channelKey: currentState?.key || null,
      channelDescriptor: currentState?.descriptor || null,
      warningMessage,
      threadReset,
  ...
  function buildPiSpawnSpec(session, options = {}) {
    ...
    const commandSpec = buildCliSpawnCommand(PI_PATH || 'pi', args);
    return {
      command: commandSpec.command,
      args: commandSpec.args,
      env,
  ...
  function imageResultToDataUrl(result, mime = 'image/png') {
  ...
  function appendEntryImageSegment(entry, image = {}) {
  ...
  function cloneRuntimeDraftSegments(segments) {
  ...
  function findRuntimeDraftMessageIndex(session, entry) {
  ...
  function persistRuntimeDraftMessage(sessionId, entry, options = {}) {
    if (!entry || entry.draftFinalized) return null;
    const hasContent = !!String(entry.fullText || '')
    ...
    if (!force && entry.lastDraftPersistAt && nowMs - entry.lastDraftPersistAt < 750) {
      entry.draftDirty = true;
      return null;
    }
    ...
    if (!entry.draftMessageId) {
      entry.draftMessageId = `runtime-draft-${entry.pid || nowMs}-${Math.random().toString(36).slice(2, 8)}`;
    }
    const timestamp = entry.draftMessageTimestamp
      || (Number(entry.startedAtMs || 0) ? new Date(entry.startedAtMs).toISOString() : new Date(nowMs).toISOString());
    entry.draftMessageTimestamp = timestamp;

    const draft = {
      role: 'assistant',
      content: entry.fullText || '',
      toolCalls: Array.isArray(entry.toolCalls) ? entry.toolCalls.map((tool) => ({ ...(tool || {}) })) : [],
      segments: cloneRuntimeDraftSegments(entry.segments),
      timestamp,
      streaming: true,
      runtimeDraftId: entry.draftMessageId,
      updatedAt: new Date(nowMs).toISOString(),
    };

    const index = findRuntimeDraftMessageIndex(session, entry);
    if (index === -1) session.messages.push(draft);
    else session.messages[index] = { ...session.messages[index], ...draft };
    session.updated = draft.updatedAt;
    saveSession(session);
    entry.lastDraftPersistAt = nowMs;
    entry.draftDirty = false;
    return session;
  }
  ...
  function persistRuntimeUsage(sessionId, entry, usage, options = {}) {
    ...
    if (options.source === 'turn_completed'
      && currentUsage
      && ((previousTotalUsage && tokenUsageSameRuntimeFields(currentUsage, previousTotalUsage))
        || (session.lastUsage && tokenUsageSameRuntimeFields(currentUsage, session.lastUsage)))) {
      currentUsage = null;
    }
  ...
  function claudeToolResultText(block) {
  ...
  function completeClaudeToolResult(entry, block, sessionId) {
    const toolUseId = block?.tool_use_id || block?.id || '';
    if (!toolUseId) return;
    const truncatedResult = truncateToolResult(claudeToolResultText(block));
    const tc = entry.toolCalls.find((tool) => tool.id === toolUseId);
    if (tc) {
      tc.done = true;
      tc.result = truncatedResult;
    }
    updateEntryToolSegment(entry, toolUseId, {
      done: true,
      result: truncatedResult,
    });
    sendEntryMessage(entry, { type: 'tool_end', sessionId, toolUseId, result: truncatedResult });
  }

  function formatClaudeApiRetryWarning(event) {
    const attempt = Number(event?.attempt || 0) || 0;
    const maxRetries = Number(event?.max_retries || event?.maxRetries || 0) || 0;
    const retryDelayMs = Number(event?.retry_delay_ms || event?.retryDelayMs || 0) || 0;
    const status = String(event?.error_status || event?.status || '').trim();
    ...
    const errorText = typeof rawError === 'string'
      ...
      : rawError?.message
        ...
        : rawError
          ...
          : '';
    const retryPart = attempt && maxRetries ? `（${attempt}/${maxRetries}` : attempt ? `（第 ${attempt} 次` : '（';
    const delayText = retryDelayMs >= 1000
      ? `${Number((retryDelayMs / 1000).toFixed(1))}秒`
      : `${Math.round(retryDelayMs)}ms`;
    const delayPart = retryDelayMs > 0 ? `，约 ${delayText}后` : '';
    const detail = [status, errorText].filter(Boolean).join(' · ');
    return `Claude API 暂时异常，正在自动重试${retryPart}${delayPart}）：${detail || '上游请求失败'}`;
  }

  function sendClaudeApiRetryWarning(entry, event, sessionId) {
    const key = `${event?.subtype || 'api_retry'}:${event?.attempt || ''}:${event?.error_status || ''}:${event?.retry_delay_ms || ''}`;
    entry.claudeRuntimeWarningKeys = entry.claudeRuntimeWarningKeys || new Set();
    if (entry.claudeRuntimeWarningKeys.has(key)) return;
    entry.claudeRuntimeWarningKeys.add(key);
    sendEntryMessage(entry, {
      type: 'runtime_warning',
      sessionId,
      message: formatClaudeApiRetryWarning(event),
    });
  }
    ...
    switch (event.type) {
      case 'system':
        ...
        if (event.subtype === 'api_retry') {
          sendClaudeApiRetryWarning(entry, event, sessionId);
        }
      ...
      case 'assistant': {
        ...
        for (const block of content) {
          if (block.type === 'text' && block.text) {
            ...
            completeClaudeToolResult(entry, block, sessionId);
          }
        }
      ...
      case 'user': {
        ...
        if (!Array.isArray(content)) break;
        for (const block of content) {
          if (block?.type === 'tool_result') completeClaudeToolResult(entry, block, sessionId);
        }
        break;
      }
  ...
  function processCodexEvent(entry, event, sessionId) {
    ...
    switch (codexEvent.type) {
      case 'item.agent_message.delta': {
        const text = String(codexEvent.text || codexEvent.delta || '');
        if (text) {
          entry.fullText += text;
          appendEntryTextSegment(entry, text);
          sendEntryMessage(entry, { type: 'text_delta', sessionId, text });
          persistRuntimeDraftMessage(sessionId, entry);
        }
        break;
      }

      case 'thread.goal.updated': {
        const goal = codexEvent.goal || null;
        sendEntryMessage(entry, {
          type: 'runtime_status',
          sessionId,
          code: 'codex_goal',
          message: goal
            ? `Codex goal: ${goal.status} · tokensUsed=${goal.tokensUsed ?? 0} · timeUsedSeconds=${goal.timeUsedSeconds ?? 0}`
            : 'Codex goal updated',
          goal,
        });
        break;
      ...
      case 'thread.started': {
        ...
        entry.codexRuntimeSessionId = codexEvent.thread_id;
        const session = loadSession(sessionId);
        if (session) {
          if (typeof setRuntimeSessionState === 'function') {
            setRuntimeSessionState(session, {
      ...
      case 'item.started': {
        const item = normalizeCodexItem(codexEvent.item);
      ...
      case 'item.updated':
      case 'item.delta': {
        const item = normalizeCodexItem(codexEvent.item || codexEvent);
        if (!item) break;
        ...
        const isAgentMessage = !item.type || item.type === 'agent_message' || codexEvent.type === 'item.delta';
      ...
      case 'item.completed': {
        ...
        persistRuntimeDraftMessage(sessionId, entry, { force: true });
        break;
      }

      case 'item.reasoning.delta': {
        const text = String(codexEvent.delta || codexEvent.item?.delta || '');
        if (!text) break;
      ...
      case 'turn.completed': {
        ...
        if (usage) persistRuntimeUsage(sessionId, entry, usage, {
          contextWindowTokens: codexEvent.model_context_window || null,
      ...
      case 'image_generation_end': {
        ...
        if (typeof createGeneratedImageSegmentFromCodexEvent === 'function') {
          image = createGeneratedImageSegmentFromCodexEvent(codexEvent, {
            sessionId,
            runtimeSessionId: entry.codexRuntimeSessionId || null,
          });
        }
        if (!image) {
          const src = imageResultToDataUrl(codexEvent.result, 'image/png');
          if (!src) break;
          image = {
            id: codexEvent.call_id || null,
            src,
            mime: 'image/png',
            alt: 'Generated image',
        ...
        appendEntryImageSegment(entry, image);
        sendEntryMessage(entry, { type: 'image_delta', sessionId, ...image });
        persistRuntimeDraftMessage(sessionId, entry, { force: true });
        break;
      }
      ...
      case 'error':
        if (codexEvent.message) {
          if (/^Reconnecting\.\.\./.test(codexEvent.message)) {
            sendSessionEvent(entry, sessionId, { type: 'system_message', message: codexEvent.message });
          } else {
