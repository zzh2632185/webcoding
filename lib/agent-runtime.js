...
function createAgentRuntime(deps) {
  ...
  function normalizeTokenUsage(raw) {
    ...
    return {
      ...
      cachedInputTokens: Number(raw.cachedInputTokens ?? raw.cached_input_tokens) || 0,
      outputTokens: Number(raw.outputTokens ?? raw.output_tokens) || 0,
      reasoningOutputTokens: Number(raw.reasoningOutputTokens ?? raw.reasoning_output_tokens) || 0,
      totalTokens: Number(raw.totalTokens ?? raw.total_tokens) || 0,
    };
  }

  function persistRuntimeUsage(sessionId, entry, usage, options = {}) {
    const currentUsage = normalizeTokenUsage(usage);
    const session = loadSession(sessionId);
    if (!session) return null;
    if (currentUsage) {
    ...
    if (options.totalUsage) {
      const totalUsage = normalizeTokenUsage(options.totalUsage);
      if (totalUsage) session.totalUsage = totalUsage;
    ...
    switch (event.type) {
      ...
      case 'assistant': {
        ...
        for (const block of content) {
          if (block.type === 'text' && block.text) {
            ...
            wsSend(entry.ws, { type: 'text_delta', sessionId, text: block.text });
            ...
            wsSend(entry.ws, { type: 'tool_end', sessionId, toolUseId: block.tool_use_id, result: truncatedResult });
          }
        }
      ...
      case 'result': {
        ...
        if (entry.ws && event.total_cost_usd !== undefined) {
          wsSend(entry.ws, { type: 'cost', sessionId, costUsd: entry.claudeSessionTotalCost || 0 });
        }
        break;
      }
    }
      ...
      case 'item.completed': {
        ...
        if (item.type === 'agent_message') {
          if (item.text) {
            ...
            wsSend(entry.ws, { type: 'text_delta', sessionId, text: item.text });
          }
          break;
        }
        if (item.type === 'error') {
          ...
          if (warningMessage) {
            wsSend(entry.ws, { type: 'system_message', sessionId, message: warningMessage });
          }
          break;
        }
      ...
      case 'error':
        if (codexEvent.message) {
          if (/^Reconnecting\.\.\./.test(codexEvent.message)) {
            wsSend(entry.ws, { type: 'system_message', sessionId, message: codexEvent.message });
          } else {
