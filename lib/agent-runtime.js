  ...
  if (hasPathSeparator) {
    ...
    if (!path.extname(raw)) {
      for (const ext of uniqueExts) {
        if (ext) candidates.push(`${raw}${ext}`);
      }
    }
  } else {
    ...
    for (const dir of pathDirs) {
      for (const ext of uniqueExts) {
        if (ext) candidates.push(path.join(dir, `${raw}${ext}`));
      }
...
function parseWindowsNpmCmdShim(commandPath) {
  ...
  while ((match = targetRe.exec(source))) {
    targets.push(path.join(basedir, match[1]));
  }
...
function createAgentRuntime(deps) {
  ...
  function buildCodexSpawnSpec(session, options = {}) {
    ...
    if (runtimeConfig?.mode === 'custom') {
      ...
      if (runtimeConfig.defaultModel) {
        appServerArgs.push('-c', `model=${quoted(runtimeConfig.defaultModel)}`);
      }
    }

    const reasoningEffort = String(session.reasoningEffort || '').trim().toLowerCase();
    if (reasoningEffort) {
      appServerArgs.push('-c', `model_reasoning_effort=${quoted(reasoningEffort)}`);
    }
    const fastModeEnabled = String(process.env.CC_WEB_CODEX_FAST_MODE_ENABLED || '').trim() === '1';
    ...
    const nativeRuntimeScript = path.join(__dirname, 'codex-native-runtime.js');
    const nativeAttachments = Array.isArray(options.attachments) ? options.attachments : [];
    const args = [nativeRuntimeScript, ...appServerArgs];
    for (const attachment of nativeAttachments) {
      if (attachment?.path) args.push('--local-image', attachment.path);
    }

    const envOverrides = {
      WEBCODING_CODEX_PATH: CODEX_PATH,
      WEBCODING_CODEX_APP_SERVER_ARGS: JSON.stringify(appServerArgs),
      WEBCODING_CODEX_THREAD_ID: runtimeId || '',
      WEBCODING_CODEX_CWD: resolveWorkingDir(session),
      WEBCODING_CODEX_MODEL: session.model || '',
      WEBCODING_CODEX_REASONING_EFFORT: reasoningEffort || '',
      WEBCODING_CODEX_FAST_MODE: fastMode ? '1' : '',
      WEBCODING_CODEX_PERMISSION_MODE: permMode,
      WEBCODING_CODEX_ATTACHMENTS: JSON.stringify(nativeAttachments),
    };
    if (runtimeConfig?.mode === 'custom') {
      envOverrides.CODEX_HOME = runtimeConfig.homeDir;
      envOverrides[CODEX_OPENAI_COMPAT_ENV_KEY] = runtimeConfig.apiKey;
      envOverrides.OPENAI_API_KEY = null;
      envOverrides.OPENAI_BASE_URL = null;
    }
    const env = sanitizeChildEnv(envOverrides);

    const commandSpec = buildCliSpawnCommand(process.execPath, args);
  ...
  function completeClaudeToolResult(entry, block, sessionId) {
    ...
    const tc = entry.toolCalls.find((t) => t.id === toolUseId);
    if (tc) {
      tc.done = true;
      ...
      case 'error':
        if (codexEvent.message) {
          if (/^Reconnecting\.\.\./.test(codexEvent.message)) {
            const match = String(codexEvent.message || '').match(/^Reconnecting\.\.\.\s*(\d+)\/(\d+)(?:\s*\((.*)\))?/);
            const message = match
              ? `Codex 上游连接中断，正在自动重连（${match[1]}/${match[2]}）…`
              : 'Codex 上游连接中断，正在自动重连…';
            sendEntryMessage(entry, { type: 'system_message', sessionId, message });
          } else {
...
module.exports = { createAgentRuntime, buildCliSpawnCommand };
