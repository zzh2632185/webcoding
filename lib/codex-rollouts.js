...
function createCodexRolloutStore(deps) {
  const { codexSessionsDir, codexRuntimeSessionsDir, sessionsDir, normalizeSession, sanitizeToolInput } = deps;
  ...
  function parseCodexRolloutLines(lines) {
    ...
    const result = { meta, messages, totalUsage: finalTotalUsage };
